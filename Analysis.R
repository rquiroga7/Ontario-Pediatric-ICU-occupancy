...
for (year in 2018:2022){
  assign(paste0("a",year), read_csv(paste0(year,"_test_data.csv")))
}
...
#Plot cases + ICU
coeff=10
ggplot(data_long4 %>% filter(Data=="Total Flu Positive" | Data=="RSV Positive" | Data=="EV/RV Positive" ),aes(x=date,y=Count,color=Data))+
  theme_light(base_size=16)+
  ggtitle("Ontario - Weekly positives for different viruses")+
  theme(legend.position = "bottom")+
  labs(caption=tag)+
...
ggsave(filename = "virus_ICU_ped_Ontario.png",width = 12, height=8, dpi= 150)

#Plot cases + ICU + COVID
coeff=10
ggplot(data_long4 %>% filter(Data=="Total Flu Positive" | Data=="RSV Positive" | Data=="EV/RV Positive" | Data=="COVID Positive" ),aes(x=date,y=Count,color=Data))+
  theme_light(base_size=16)+
  ggtitle("Ontario - Weekly positives for different viruses")+
  theme(legend.position = "bottom")+
  labs(caption=tag)+
  ...
  theme(
    axis.title.y = element_text(color = "black", size=13),
    axis.title.y.right = element_text(color = "#00BF7D", size=13)
  )
ggsave(filename = "virus_COVID_ICU_ped_Ontario.png",width = 12, height=8, dpi= 150)
